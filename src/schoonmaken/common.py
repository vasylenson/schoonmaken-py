...
class Counters:
    ...
    def week_since_any_task(self, person):
        task_times = ((task, person) for task in self.tasks)
        return min(self.weeks_since_done[time] for time in task_times)

    def score(self, task: Task, person: Person) -> float:
        td = self.times_done[task, person]
        wsd = self.weeks_since_done[task, person]
        wsat = self.week_since_any_task(person)
        if task.ismonthly:
            wsd /= 8
            td *= 2
        return td + 5 / wsd + 5 / wsat
