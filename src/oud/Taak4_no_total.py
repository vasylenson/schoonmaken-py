...
### Settings
all_persons = [101, 102, 103, 104, 105, 106, 107, 211, 212, 213, 214, 215, 216, 217]
...
for n in range(0,len(persons_no_task)):
    if persons_no_task[n] != 0:
        if persons_no_task[n] not in all_persons:
            ...
            persons_no_task_nonzero_ind = persons_no_task_nonzero_ind + [all_persons.index(persons_no_task[n])]
...
old_noclean_weeks = np.zeros((nr_of_people,1),dtype=int) #amount of weeks that each person did not clean

datevector = np.zeros((nr_of_weeks_new,1),dtype=int) #vector for new dates
datevector_str = [] #vector for new dates
first_of_month = np.zeros((nr_of_weeks_new,1),dtype=int) #bool if this week of the month
...
for current_week in range(0,len(oldlist[:,0])):
    for current_task in range(0,nr_of_tasks):
        ...
        if roomnr_oldtask != 0 and roomnr_oldtask != -1:
            # Find index of person
            pers_ind = all_persons.index(roomnr_oldtask)
            task_counters[pers_ind,current_task] = task_counters[pers_ind,current_task] + 1.0
...
weekly_chance_old = nr_of_weekly_tasks_done_old/(nr_of_people*nr_of_weeks_old-np.sum(old_noclean_weeks)) 
...
monthly_chance_old = nr_of_monthly_tasks_done_old/(nr_of_people*nr_of_weeks_old-np.sum(old_noclean_weeks))
...
for current_week in range(0,nr_of_weeks_new):
    ...
    datevector_str = datevector_str + [date.fromordinal(t).strftime("%d-%m-%Y")]
...
for current_week in range(0,nr_of_weeks_new):
    ...
    for current_task in range(0,nr_of_tasks):
        ...
        if (current_task < nr_of_tasks- nr_of_monthly_tasks) or (first_of_month[current_week] == 1):
            ...
            for n in range(0,len(persons_no_task)):
                if persons_no_task[n] != 0:
                    cleaning_score[all_persons.index(persons_no_task[n])] = np.inf
            
            # find possibilities
            lowest_counter = np.min(cleaning_score)
            lowest_ind = np.arange(0,nr_of_people)[cleaning_score==lowest_counter] #lowest indices
            #round cleaning scores because of decimals
            random_of_lowest = np.random.randint(0,len(lowest_ind)) #choose random person
            lowest_person = lowest_ind[random_of_lowest]
            
            
            task_counters[lowest_person,current_task] = task_counters[lowest_person,current_task] + 1
            current_week_counter[lowest_person] = current_week_counter[lowest_person] + 1
            if current_task==hallways:
                task_counters[lowest_person,current_task] = task_counters[lowest_person,current_task] - 0.5
                current_week_counter[lowest_person] = current_week_counter[lowest_person] - 0.5
            
            tasks[current_week,current_task] = lowest_person

### update total task counters for people not doing task
total_task_counter = np.sum(task_counters,1)
total_task_counter_diff = np.sum(total_task_counter) - np.sum(total_task_counter_old)
tasks_per_pers = total_task_counter_diff/(nr_of_people-len(persons_no_task_nonzero_ind))
total_task_counter[persons_no_task_nonzero_ind] = total_task_counter[persons_no_task_nonzero_ind] + tasks_per_pers
...
newlist[:,nr_of_tasks+1:nr_of_tasks+1+nr_no_task] = np.ones((nr_of_weeks_new,1),dtype=int)*persons_no_task

colnames = ['Date', 'Living', 'room', 'Toilets', 'Bathroom', 'Showers', 'Hallways', 'Kit', 'ch', 'en', 'Laundry', 'Notask1', 'Notask2', 'Notask3','Notask4','Notask5']
outtable = pd.DataFrame(newlist,    # values
